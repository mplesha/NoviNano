...
window.CMB = (function(window, document, $, undefined){
	...
	var l10n = window.cmb_Closify_l10;
	...
	var cmb = {
		...
		idNumber    : false,
		file_frames : {},
		repeatEls   : 'input:not([type="button"]),select,textarea,.cmb_Closify_media_status'
	};
	...
	cmb.metabox = function() {
		...
		cmb.$metabox = $('table.cmb_Closify_metabox');
		return cmb.$metabox;
	};
	...
	cmb.init = function() {
		 ...
		 */
		cmb.initPickers( $metabox.find('input:text.cmb_Closify_timepicker'), $metabox.find('input:text.cmb_Closify_datepicker'), $metabox.find('input:text.cmb_Closify_colorpicker') );

		// Wrap date picker in class to narrow the scope of jQuery UI CSS and prevent conflicts
		$("#ui-datepicker-div").wrap('<div class="cmb_Closify_element" />');

		// Insert toggle button into DOM wherever there is multicheck. credit: Genesis Framework
		$( '<p><span class="button cmb-multicheck-toggle">' + l10n.check_toggle + '</span></p>' ).insertBefore( 'ul.cmb_Closify_checkbox_list' );

		$metabox
			.on( 'change', '.cmb_Closify_upload_file', function() {
				cmb.formfield = $(this).attr('id');
				$('#' + cmb.formfield + '_id').val('');
			})
			// Media/file management
			.on( 'click', '.cmb-multicheck-toggle', cmb.toggleCheckBoxes )
			.on( 'click', '.cmb_Closify_upload_button', cmb.handleMedia )
			.on( 'click', '.cmb_Closify_remove_file_button', cmb.handleRemoveMedia )
			// Repeatable content
			...
			// Ajax oEmbed display
			.on( 'keyup paste focusout', '.cmb_Closify_oembed', cmb.maybeOembed )
			// Reset titles when removing a row
			.on( 'cmb_Closify_remove_row', '.repeatable-group', cmb.resetTitlesAndIterator );
		...
		if ( $repeatGroup.length ) {
			$repeatGroup
				...
				.on( 'cmb_Closify_add_row', cmb.emptyValue );
		}
	...
	cmb.handleMedia = function(event) {
		...
		var attachment   = true;
		var isList       = $self.hasClass( 'cmb_Closify_upload_list' );
		...
		var handlers = {
			list : function( selection ) {
				...
				$( attachment ).each( function() {
					if ( this.type && this.type === 'image' ) {
						...
						uploadStatus = '<li class="img_status">'+
							...
							'<p><a href="#" class="cmb_Closify_remove_file_button" rel="'+ cmb.formfield +'['+ this.id +']">'+ l10n.remove_image +'</a></p>'+
							'<input type="hidden" id="filelist-'+ this.id +'" name="'+ formName +'['+ this.id +']" value="' + this.url + '">'+
						'</li>';

					} else {
						// Standard generic output if it's not an image.
						uploadStatus = '<li>'+ l10n.file +' <strong>'+ this.filename +'</strong>&nbsp;&nbsp;&nbsp; (<a href="' + this.url + '" target="_blank" rel="external">'+ l10n.download +'</a> / <a href="#" class="cmb_Closify_remove_file_button" rel="'+ cmb.formfield +'['+ this.id +']">'+ l10n.remove_file +'</a>)'+
							'<input type="hidden" id="filelist-'+ this.id +'" name="'+ formName +'['+ this.id +']" value="' + this.url + '">'+
						'</li>';
				...
				// Append each item from our fileGroup array to .cmb_Closify_media_status
				$( fileGroup ).each( function() {
					$formfield.siblings('.cmb_Closify_media_status').slideDown().append(this);
				});
			},
			single : function( selection ) {
				...
				if ( attachment.type && attachment.type === 'image' ) {
					// image preview
					uploadStatus = '<div class="img_status"><img style="max-width: 350px; width: 100%; height: auto;" src="' + attachment.url + '" alt="'+ attachment.filename +'" title="'+ attachment.filename +'" /><p><a href="#" class="cmb_Closify_remove_file_button" rel="' + cmb.formfield + '">'+ l10n.remove_image +'</a></p></div>';
				} else {
					// Standard generic output if it's not an image.
					uploadStatus = l10n.file +' <strong>'+ attachment.filename +'</strong>&nbsp;&nbsp;&nbsp; (<a href="'+ attachment.url +'" target="_blank" rel="external">'+ l10n.download +'</a> / <a href="#" class="cmb_Closify_remove_file_button" rel="'+ cmb.formfield +'">'+ l10n.remove_file +'</a>)';
				}

				// add/display our output
				$formfield.siblings('.cmb_Closify_media_status').slideDown().html(uploadStatus);
			}
		};
	...
	cmb.handleRemoveMedia = function( event ) {
		event.preventDefault();
		var $self = $(this);
		if ( $self.is( '.attach_list .cmb_Closify_remove_file_button' ) ){
		...
		if ( ! $container.length ) {
			$self.parents('.cmb_Closify_media_status').html('');
		} else {
			$container.html('');
		}
		return false;
	...
	$.fn.newRowHousekeeping = function() {
		...
		var $list        = $row.find( '.cmb_Closify_media_status' );

		if ( $colorPicker.length ) {
			// Need to clean-up colorpicker before appending
			$colorPicker.each( function() {
				var $td = $(this).parent();
				$td.html( $td.find( 'input:text.cmb_Closify_colorpicker' ).attr('style', '') );
			});
		}
	...
	cmb.afterRowInsert = function( $row ) {
		...
		cmb.initPickers( $row.find('input:text.cmb_Closify_timepicker'), $row.find('input:text.cmb_Closify_datepicker'), $row.find('input:text.cmb_Closify_colorpicker') );
	};
	...
	cmb.addGroupRow = function( event ) {
		...
		$table.trigger( 'cmb_Closify_add_row', $newRow );
	};
	...
	cmb.addAjaxRow = function( event ) {
		...
		$table.trigger( 'cmb_Closify_add_row', $row );
	};
	...
	cmb.removeGroupRow = function( event ) {
		...
		if ( noRows > 1 ) {
			...
			$table.trigger( 'cmb_Closify_remove_row' );
		}
	};
	...
	cmb.shiftRows = function( event ) {
		...
		$parent.find( cmb.repeatEls ).each( function() {
			var $element = $(this);
			var val;
			if ( $element.hasClass('cmb_Closify_media_status') ) {
		...
		var m = {
			focusout : function() {
				setTimeout( function() {
					...
					cmb.spinner( '.postbox table.cmb_Closify_metabox', true );
				}, 2000);
			},
			keyup : function() {
	...
	cmb.resizeoEmbeds = function() {
		cmb.metabox().each( function() {
			...
			var $children = $embeds.children().not('.cmb_Closify_remove_wrapper');
			if ( ! $children.length ) {
				return true; // continue
			}
	...
	cmb.doAjax = function($obj) {
		...
		// only proceed if the user has pasted, pressed a number, letter, or whitelisted characters
			...
			$context = $context.length ? $context : $obj.parents('.cmb_Closify_metabox tr td');
			...
			setTimeout( function() {
				...
				if ( $('.cmb_Closify_oembed:focus').val() !== oembed_url ) {
					return;
				}
				$.ajax({
					type : 'post',
					...
					url : l10n.ajaxurl,
					data : {
						'action': 'cmb_Closify_oembed_handler',
						'oembed_url': oembed_url,
						'oembed_width': oembed_width > 300 ? oembed_width : 300,
						'field_id': field_id,
						'object_id': $obj.data('objectid'),
						'object_type': $obj.data('objecttype'),
						'cmb_Closify_ajax_nonce': l10n.ajax_nonce
					},
