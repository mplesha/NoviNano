...
spl_autoload_register('Closify_Meta_Box::autoload_helpers');

$meta_boxes = array();
$meta_boxes = apply_filters( 'Closify_Meta_Boxes', $meta_boxes );
foreach ( $meta_boxes as $meta_box ) {
	$my_box = new Closify_Meta_Box( $meta_box );
}

define( 'Closify_Meta_Box_URL', Closify_Meta_Box::get_meta_box_url() );

/**
 * Create meta boxes
 */
class Closify_Meta_Box {
	 ...
	 * @since 1.0.0
	 */
	const cmb_Closify_VERSION = '1.2.0';
	...
	protected static $mb_defaults = array(
		...
		'cmb_Closify_styles' => true, // Include cmb bundled stylesheet
		'fields'     => array(),
	);
	...
	function __construct( $meta_box ) {
		...
		$allow_frontend = apply_filters( 'cmb_Closify_allow_frontend', true, $meta_box );
		...
		$show_filters = 'Closify_Meta_Box_Show_Filters';
		foreach ( get_class_methods( $show_filters ) as $filter ) {
			add_filter( 'cmb_Closify_show_on', array( $show_filters, $filter ), 10, 2 );
		}
	...
	public function register_scripts() {
		...
		if ( 3.5 <= $wp_version ) {
			...
			if ( ! is_admin() ) {
			   ...
			   wp_register_script( 'iris', admin_url( 'js/iris.min.js' ), array( 'jquery-ui-draggable', 'jquery-ui-slider', 'jquery-touch-punch' ), self::cmb_Closify_VERSION );
		   	wp_register_script( 'wp-color-picker', admin_url( 'js/color-picker.min.js' ), array( 'iris' ), self::cmb_Closify_VERSION );
		...
		wp_register_script( 'cmb-datepicker', Closify_Meta_Box_URL . 'js/jquery.datePicker.min.js' );
		wp_register_script( 'cmb-timepicker', Closify_Meta_Box_URL . 'js/jquery.timePicker.min.js' );
		wp_register_script( 'cmb-scripts', Closify_Meta_Box_URL .'js/cmb'. $min .'.js', $scripts, self::cmb_Closify_VERSION );

		wp_enqueue_media();

		wp_localize_script( 'cmb-scripts', 'cmb_Closify_l10', apply_filters( 'cmb_Closify_localized_data', array(
		...
		wp_register_style( 'cmb-styles', Closify_Meta_Box_URL . 'style'. $min .'.css', $styles );
	...
	public function do_scripts( $hook ) {
		...
		if ( $hook == 'post.php' || $hook == 'post-new.php' || $hook == 'page-new.php' || $hook == 'page.php' ) {
			...
			if ( $this->_meta_box['cmb_Closify_styles'] )
	...
	public function add_metaboxes() {
		...
		foreach ( $this->_meta_box['pages'] as $page ) {
			if ( apply_filters( 'cmb_Closify_show_on', true, $this->_meta_box ) )
	...
	public function user_metabox() {
		...
		if ( $this->_meta_box['cmb_Closify_styles'] != false )
			wp_enqueue_style( 'cmb-styles' );
	...
	public static function show_form( $meta_box, $object_id = 0, $object_type = '' ) {
		...
		echo "\n<!-- Begin CMB Fields -->\n";
		do_action( 'cmb_Closify_before_table', $meta_box, $object_id, $object_type );
		echo '<table class="form-table cmb_Closify_metabox">';
		...
		foreach ( $meta_box['fields'] as $field_args ) {
			...
			if ( 'group' == $field_args['type'] ) {
				...
				$field = new Closify_Meta_Box_field( $field_args );
				$field->render_field();
			}
		}
		echo '</table>';
		do_action( 'cmb_Closify_after_table', $meta_box, $object_id, $object_type );
		echo "\n<!-- End CMB Fields -->\n";
	...
	public static function render_group( $args ) {
		...
		$args['count']   = 0;
		$field_group     = new Closify_Meta_Box_field( $args );
		...
		if ( $desc || $label ) {
			echo '<tr><th>';
				...
				if ( $desc )
					echo '<p class="cmb_Closify_metabox_description">'. $desc .'</p>';
			echo '</th></tr>';
		}
		...
		<tr class="repeatable-grouping" data-iterator="'. $field_group->count() .'">
			<td>
				...
				foreach ( array_values( $field_group->args( 'fields' ) ) as $field_args ) {
					...
					$field = new Closify_Meta_Box_field( $field_args, $field_group );
					$field->render_field();
				}
				echo '
					<tr>
	...
	public static function save_fields( $meta_box, $object_id, $object_type = '' ) {
		...
		if ( ! apply_filters( 'cmb_Closify_show_on', true, $meta_box ) )
			return;
		...
		do_action( "cmb_Closify_save_{$object_type}_fields", $object_id, $meta_box['id'], self::$updated, $meta_box );
	...
	public static function get_meta_box_url() {
		...
		if ( strtoupper( substr( PHP_OS, 0, 3 ) ) === 'WIN' ) {
			...
			$cmb_Closify_url = str_replace( DIRECTORY_SEPARATOR, '/', $content_url );

		} else {
		  $cmb_Closify_url = str_replace(
		...
		return trailingslashit( apply_filters('Closify_Meta_Box_url', $cmb_Closify_url ) );
	}
	 ...
	 * @uses apply_filters() Calls 'cmb_Closify_override_option_get_$option_key' hook to allow
	...
	public static function _get_option( $option_key, $default = false ) {
		...
		$test_get = apply_filters( "cmb_Closify_override_option_get_$option_key", 'cmb_Closify_no_override_option_get', $default );

		if ( $test_get !== 'cmb_Closify_no_override_option_get' )
			return $test_get;
	 ...
	 * @uses apply_filters() Calls 'cmb_Closify_override_option_save_$option_key' hook to allow
	...
	public static function save_option( $option_key ) {
		...
		$test_save = apply_filters( "cmb_Closify_override_option_save_$option_key", 'cmb_Closify_no_override_option_save', $to_save );

		if ( $test_save !== 'cmb_Closify_no_override_option_save' )
			return $test_save;
...
// Handle oembed Ajax
add_action( 'wp_ajax_cmb_Closify_oembed_handler', array( 'Closify_Meta_Box_ajax', 'oembed_handler' ) );
add_action( 'wp_ajax_nopriv_cmb_Closify_oembed_handler', array( 'Closify_Meta_Box_ajax', 'oembed_handler' ) );
 ...
 * @return array               Options array or specific field
 */
function cmb_Closify_get_option( $option_key, $field_id = '' ) {
	return Closify_Meta_Box::get_option( $option_key, $field_id );
}
 ...
 * @param  int    $object_id   Object ID
 * @param  string $object_type Type of object being saved. (e.g., post, user, or comment)
 * @return object              Closify_Meta_Box_field object
 */
function cmb_Closify_get_field( $field_args, $object_id = 0, $object_type = 'post' ) {
	// Default to the loop post ID
	$object_id = $object_id ? $object_id : get_the_ID();
	Closify_Meta_Box::set_object_id( $object_id );
	Closify_Meta_Box::set_object_type( $object_type );
	// Send back field object
	return new Closify_Meta_Box_field( $field_args );
}
 ...
 */
function cmb_Closify_get_field_value( $field_args, $object_id = 0, $object_type = 'post' ) {
	$field = cmb_Closify_get_field( $field_args, $object_id, $object_type );
 ...
 * @param int   $object_id  Object ID
 */
function cmb_Closify_print_metaboxes( $meta_boxes, $object_id ) {
	foreach ( (array) $meta_boxes as $meta_box ) {
		cmb_Closify_print_metabox( $meta_box, $object_id );
	}
}
 ...
 * @param int   $object_id Object ID
 */
function cmb_Closify_print_metabox( $meta_box, $object_id ) {
	$cmb = new Closify_Meta_Box( $meta_box );
	if ( $cmb ) {
		...
		if ( $meta_box['cmb_Closify_styles'] != false )
			wp_enqueue_style( 'cmb-styles' );

		Closify_Meta_Box::show_form( $meta_box );
	}
 ...
 * @param int   $object_id Object ID
 */
function cmb_Closify_save_metabox_fields( $meta_box, $object_id ) {
	Closify_Meta_Box::save_fields( $meta_box, $object_id );
}
 ...
 */
function cmb_Closify_metabox_form( $meta_box, $object_id, $echo = true ) {

	$meta_box = Closify_Meta_Box::set_mb_defaults( $meta_box );
	...
	Closify_Meta_Box::set_object_type( Closify_Meta_Box::set_mb_type( $meta_box ) );
	...
	if (
		...
		&& wp_verify_nonce( $_POST['wp_meta_box_nonce'], Closify_Meta_Box::nonce() )
		&& $_POST['object_id'] == $object_id
	)
		cmb_Closify_save_metabox_fields( $meta_box, $object_id );
	...
	$form_format = apply_filters( 'cmb_Closify_frontend_form_format', '<form class="cmb-form" method="post" id="%s" enctype="multipart/form-data" encoding="multipart/form-data"><input type="hidden" name="object_id" value="%s">%s<input type="submit" name="submit-cmb" value="%s" class="button-primary"></form>', $object_id, $meta_box, $form );
