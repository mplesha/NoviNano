...
/**
 * Show On Filters
 * Use the 'cmb_Closify_show_on' filter to further refine the conditions under which a metabox is displayed.
 ...
 * @since  1.0.0
 */
class Closify_Meta_Box_Show_Filters {
	...
	public static function check_id( $display, $meta_box ) {
		...
		$object_id = is_admin() ? Closify_Meta_Box::get_object_id() : @get_the_id();
	...
	public static function check_page_template( $display, $meta_box ) {
		...
		$object_id = Closify_Meta_Box::get_object_id();

		if ( ! $object_id || Closify_Meta_Box::get_object_type() !== 'post' )
			return false;
