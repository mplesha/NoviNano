 ...
 * @since  0.0.4
 */
class Closify_Meta_Box_Sanitize {

	/**
	 * A CMB field object
	 * @var Closify_Meta_Box_field object
	 */
	public $field;
	...
	public function default_sanitization( $value ) {
		...
		$updated = apply_filters( 'cmb_Closify_validate_'. $this->field->type(), null, $value, $this->object_id, $this->field->args(), $this );
	...
	public function text_datetime_timestamp_timezone( $value, $repeat = false ) {
		...
		if ( empty( $tzstring ) )
			$tzstring = Closify_Meta_Box::timezone_string();

		$offset = Closify_Meta_Box::timezone_offset( $tzstring, true );
	...
	public function _save_file_id( $value ) {
		...
		$field      = new Closify_Meta_Box_field( $args, $group );
		$id_key     = $field->_id();
		...
		if ( $value && ! $id_val ) {
			$id_val = Closify_Meta_Box::image_id_from_url( $value );
		}
