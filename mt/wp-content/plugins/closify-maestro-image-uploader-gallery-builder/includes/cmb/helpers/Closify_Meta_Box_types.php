...
class Closify_Meta_Box_types {
	...
	/**
	 * Default fallback. Allows rendering fields via "cmb_Closify_render_$name" hook
	 * @since  1.0.0
	...
	public function __call( $name, $arguments ) {
		...
		do_action( "cmb_Closify_render_$name", $this->field->args(), $this->field->escaped_value(), $this->field->object_id, $this->field->object_type, $this );
	}
	...
	public function is_valid_img_ext( $file ) {
		...
		$this->valid = empty( $this->valid )
			? (array) apply_filters( 'cmb_Closify_valid_img_types', array( 'jpg', 'jpeg', 'png', 'gif', 'ico', 'icon' ) )
			: $this->valid;
	...
	public function parse_args( $args, $element, $defaults ) {
		return wp_parse_args( apply_filters( "cmb_Closify_{$element}_attributes", $this->field->maybe_set_attributes( $args ), $this->field, $this ), $defaults );
	}
	...
	public function _desc( $paragraph = false, $echo = false ) {
		...
		$desc = "\n<$tag class=\"cmb_Closify_metabox_description\">{$this->field->args( 'description' )}</$tag>\n";
		if ( $echo )
			echo $desc;
		return $desc;
	}
	...
	public function text_small() {
		return $this->input( array( 'class' => 'cmb_Closify_text_small', 'desc' => $this->_desc() ) );
	}

	public function text_medium() {
		return $this->input( array( 'class' => 'cmb_Closify_text_medium', 'desc' => $this->_desc() ) );
	}

	public function text_email() {
		return $this->input( array( 'class' => 'cmb_Closify_text_email cmb_Closify_text_medium', 'type' => 'email' ) );
	}

	public function text_url() {
		return $this->input( array( 'class' => 'cmb_Closify_text_url cmb_Closify_text_medium regular-text', 'value' => $this->field->escaped_value( 'esc_url' ) ) );
	}

	public function text_date() {
		return $this->input( array( 'class' => 'cmb_Closify_text_small cmb_Closify_datepicker', 'desc' => $this->_desc() ) );
	}

	public function text_time() {
		return $this->input( array( 'class' => 'cmb_Closify_timepicker text_time', 'desc' => $this->_desc() ) );
	}

	public function text_money() {
		return ( ! $this->field->args( 'before' ) ? '$ ' : ' ' ) . $this->input( array( 'class' => 'cmb_Closify_text_money', 'desc' => $this->_desc() ) );
	}

	public function textarea_small() {
		return $this->textarea( array( 'class' => 'cmb_Closify_textarea_small', 'rows' => 4 ) );
	}

	public function textarea_code() {
		return sprintf( '<pre>%s</pre>', $this->textarea( array( 'class' => 'cmb_Closify_textarea_code' )  ) );
	}
	...
	public function text_date_timestamp() {
		...
		return $this->input( array( 'class' => 'cmb_Closify_text_small cmb_Closify_datepicker', 'value' => $value ) );
	}
	...
	public function text_datetime_timestamp( $meta_value = '' ) {
		...
		$inputs = array(
			$this->input( array(
				'class' => 'cmb_Closify_text_small cmb_Closify_datepicker',
			...
			$this->input( array(
				'class' => 'cmb_Closify_timepicker text_time',
	...
	public function select_timezone() {
		$this->field->args['default'] = $this->field->args( 'default' )
			...
			: Closify_Meta_Box::timezone_string();
	...
	public function colorpicker() {
		...
		return $this->input( array( 'class' => 'cmb_Closify_colorpicker cmb_Closify_text_small', 'value' => $meta_value ) );
	}

	public function title() {
		extract( $this->parse_args( array(), 'title', array(
			'tag'   => $this->field->object_type == 'post' ? 'h5' : 'h3',
			'class' => 'cmb_Closify_metabox_title',
	...
	public function radio( $args = array(), $type = 'radio' ) {
		extract( $this->parse_args( $args, $type, array(
			'class'   => 'cmb_Closify_radio_list cmb_Closify_list',
	...
	public function multicheck( $type = 'checkbox' ) {
		return $this->radio( array( 'class' => 'cmb_Closify_checkbox_list cmb_Closify_list', 'options' => $this->concat_options( array( 'type' => 'checkbox', 'name' => $this->_name() .'[]' ), 'list_input_checkbox' ) ), $type );
	}
	...
	public function checkbox() {
		$meta_value = $this->field->escaped_value();
		$args = array( 'type' => 'checkbox', 'class' => 'cmb_Closify_option cmb_Closify_list', 'value' => 'on', 'desc' => '' );
		if ( ! empty( $meta_value ) ) {
			$args['checked'] = 'checked';
		}
	...
	public function taxonomy_multicheck() {
		...
		return $this->radio( array( 'class' => 'cmb_Closify_checkbox_list cmb_Closify_list', 'options' => $options ), 'taxonomy_multicheck' );
	}
	...
	public function file_list() {
		...
		echo $this->input( array(
			'type'  => 'hidden',
			'class' => 'cmb_Closify_upload_file cmb_Closify_upload_list',
			'size'  => 45, 'desc'  => '', 'value'  => '',
		) ),
		$this->input( array(
			'type'  => 'button',
			'class' => 'cmb_Closify_upload_button button cmb_Closify_upload_list',
			'value'  => __( 'Add or Upload File', 'cmb' ),
			'name'  => '', 'id'  => '',
		) );

		echo '<ul id="', $this->_id( '_status' ) ,'" class="cmb_Closify_media_status attach_list">';
			...
			foreach ( $meta_value as $id => $fullurl ) {
				...
				if ( $this->is_valid_img_ext( $fullurl ) ) {
					...
					'<li class="img_status">',
						...
						'<p class="cmb_Closify_remove_wrapper"><a href="#" class="cmb_Closify_remove_file_button">'. __( 'Remove Image', 'cmb' ) .'</a></p>
						'. $id_input .'
					</li>';
					...
					echo
					'<li>',
						__( 'File:', 'cmb' ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', $fullurl, '" target="_blank" rel="external">'. __( 'Download', 'cmb' ) .'</a> / <a href="#" class="cmb_Closify_remove_file_button">'. __( 'Remove', 'cmb' ) .'</a>)
						'. $id_input .'
					</li>';
				}
			}
	...
	public function file() {
		...
		echo $this->input( array(
			'type'  => $input_type,
			'class' => 'cmb_Closify_upload_file',
			'size'  => 45,
			'desc'  => '',
		) ),
		'<input class="cmb_Closify_upload_button button" type="button" value="'. __( 'Add or Upload File', 'cmb' ) .'" />',
		$this->_desc( true );
		...
		$this->field = new Closify_Meta_Box_field( $args, $this->field->group );
		...
		if ( $meta_value && ! $_id_value ) {
			$_id_value = Closify_Meta_Box::image_id_from_url( esc_url_raw( $meta_value ) );
		}

		echo $this->input( array(
			'type'  => 'hidden',
			'class' => 'cmb_Closify_upload_file_id',
			'value' => $_id_value,
			'desc'  => '',
		) ),
		'<div id="', $this->_id( '_status' ) ,'" class="cmb_Closify_media_status">';
			if ( ! empty( $meta_value ) ) {

				if ( $this->is_valid_img_ext( $meta_value ) ) {
					echo '<div class="img_status">';
					echo '<img style="max-width: 350px; width: 100%; height: auto;" src="', $meta_value, '" alt="" />';
					echo '<p class="cmb_Closify_remove_wrapper"><a href="#" class="cmb_Closify_remove_file_button" rel="', $cached_id, '">'. __( 'Remove Image', 'cmb' ) .'</a></p>';
					echo '</div>';
				} else {
					...
					echo __( 'File:', 'cmb' ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', $meta_value, '" target="_blank" rel="external">'. __( 'Download', 'cmb' ) .'</a> / <a href="#" class="cmb_Closify_remove_file_button" rel="', $cached_id, '">'. __( 'Remove', 'cmb' ) .'</a>)';
				}
			}
		echo '</div>';
	}
	...
	public function oembed() {
		echo $this->input( array(
			'class'           => 'cmb_Closify_oembed regular-text',
		...
		'<div id="',$this->_id( '_status' ) ,'" class="cmb_Closify_media_status ui-helper-clearfix embed_wrap">';

			if ( $meta_value = $this->field->escaped_value() ) {
				echo Closify_Meta_Box_ajax::get_oembed( $meta_value, $this->field->object_id, array(
