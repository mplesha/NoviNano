...
class Closify_Meta_Box_ajax {
	 ...
	 * @since  0.1.0
	 * @return Closify_Meta_Box_ajax A single instance of this class.
	 */
	...
	public function oembed_handler() {
		...
		// verify our nonce
		if ( ! ( isset( $_REQUEST['cmb_Closify_ajax_nonce'], $_REQUEST['oembed_url'] ) && wp_verify_nonce( $_REQUEST['cmb_Closify_ajax_nonce'], 'ajax_nonce' ) ) )
			die();
	...
	public static function get_oembed( $url, $object_id, $args = array() ) {
		...
		if ( isset( $args['object_type'] ) && $args['object_type'] != 'post' ) {
			...
			add_filter( 'get_post_metadata', array( 'Closify_Meta_Box_ajax', 'hijack_oembed_cache_get' ), 10, 3 );
			// Sets ombed cache in our object's meta (vs postmeta)
			add_filter( 'update_post_metadata', array( 'Closify_Meta_Box_ajax', 'hijack_oembed_cache_set' ), 10, 4 );
		...
		if ( $check_embed && $check_embed != $fallback )
			return '<div class="embed_status">'. $check_embed .'<p class="cmb_Closify_remove_wrapper"><a href="#" class="cmb_Closify_remove_file_button" rel="'. $args['field_id'] .'">'. __( 'Remove Embed', 'cmb' ) .'</a></p></div>';
	...
	public static function hijack_oembed_cache_get( $check, $object_id, $meta_key ) {
		...
		$data = 'options-page' === self::$object_type
			? Closify_Meta_Box::get_option( self::$object_id, self::$embed_args['cache_key'] )
	...
	public static function hijack_oembed_cache_set( $check, $object_id, $meta_key, $meta_value ) {
		...
		if ( 'options-page' === self::$object_type ) {
			// Set the option
			Closify_Meta_Box::update_option( self::$object_id, self::$embed_args['cache_key'], $meta_value, array( 'type' => 'oembed' ) );
			// Save the option
			Closify_Meta_Box::save_option( self::$object_id );
		} else {
