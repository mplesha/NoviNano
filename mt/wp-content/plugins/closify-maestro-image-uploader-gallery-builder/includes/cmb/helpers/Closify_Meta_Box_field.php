...
class Closify_Meta_Box_field {
	...
	public function __construct( $field_args, $group_field = null ) {
		$this->object_id   = Closify_Meta_Box::get_object_id();
		$this->object_type = Closify_Meta_Box::get_object_type();
		...
		// (assuming no one would want to save 'cmb_Closify_no_override_val' as a value)
		$this->value = apply_filters( 'cmb_Closify_override_meta_value', 'cmb_Closify_no_override_val', $this->object_id, $this->args(), $this->object_type, $this );

		// If no override, get our meta
		$this->value = 'cmb_Closify_no_override_val' === $this->value
			? $this->get_data()
			: $this->value;
	}
	...
	public function get_data( $field_id = '', $args = array() ) {
		...
		$data = 'options-page' === $type
			? Closify_Meta_Box::get_option( $id, $field_id )
	...
	public function update_data( $new_value, $single = true ) {
		...
		if ( 'options-page' === $type )
			return Closify_Meta_Box::update_option( $id, $field_id, $new_value, $single );
	...
	public function remove_data( $old = '' ) {
		...
		return 'options-page' === $type
			? Closify_Meta_Box::remove_option( $id, $field_id )
	...
	public function sanitization_cb( $meta_value ) {
		...
		$clean = new Closify_Meta_Box_Sanitize( $this, $meta_value );
		// Validation via 'Closify_Meta_Box_Sanitize' (with fallback filter)
	...
	public function escaped_value( $func = 'esc_attr', $meta_value = '' ) {
		...
		$esc = apply_filters( 'cmb_Closify_types_esc_'. $this->type(), null, $meta_value, $this->args(), $this );
		if ( null !== $esc ) {
			return $esc;
		}
	...
	public function field_timezone_offset() {
		return Closify_Meta_Box::timezone_offset( $this->field_timezone() );
	}
	...
	public function render_field() {
		...
		$classes   .= ' cmb_Closify_id_'. sanitize_html_class( $this->id() );
		...
		$this_type = new Closify_Meta_Box_types( $this );
		$this_type->render();
	...
	public function _set_field_defaults( $args ) {
		...
		$args['default']    = apply_filters( 'cmb_Closify_default_filter', $args['default'], $args, $this->object_type, $this->object_type );
