  ...
  function save_category_custom_fields() {
	...
	*/	
	function page_title_bar($content){
			...
			if((is_singular() || is_home()) && $postid>0){
				$hide_page_title_bar = get_post_meta($postid, '_ccmb_hide_page_title_bar', true);
	...
	*/
	...
	function page_sidebar_layout( $content ){
			...
			if((is_singular()||is_home()) && $postid>0){
				...
				$sidebar_layout = get_post_meta($postid , '_ccmb_sidebar', true);
	...
	*/
	function enqueue_admin_scripts()
	{
		...
		if(isset($_GET['page']) && $_GET['page']=='cactus-template'){
			wp_enqueue_script( 'plugin-install' );
			wp_enqueue_script( 'updates' );
		}
		wp_enqueue_style( 'cactus-companion-admin', plugins_url('assets/css/admin.css', __FILE__));
		wp_enqueue_script( 'cactus-companion-admin', plugins_url('assets/js/admin.js', __FILE__),array('jquery','wp-color-picker' ),'',true);
		wp_localize_script( 'cactus-companion-admin', 'cactus_companion_admin',
				array(
					'ajaxurl' => admin_url('admin-ajax.php'),
					'nonce' => wp_create_nonce( 'wp_rest' ),
					'i18n' =>array('t1'=> __( 'Install and Import', 'cactus-companion' ),'t2'=> __( 'Import', 'cactus-companion' ) ),
				) );
	}
	...
	*/
	...
	function enqueue_scripts()
	{
		...
		if($postid>0){
			$this->slider = get_post_meta($postid, '_ccmb_slideshow', true);
			$bg_color = get_post_meta($postid, '_ccmb_bg_color', true);
			$bg_image = get_post_meta($postid, '_ccmb_bg_image', true);
	...
	*/
	function plugin_menu() {
		add_menu_page( 'Cactus Companion', 'Cactus Companion', 'manage_options', 'cactus-companion', array($this , 'plugin_options') );
		add_submenu_page(
			'cactus-companion', __( 'Cactus Template Directory', 'cactus-companion' ), __( 'Template Directory', 'cactus-companion' ), 'manage_options', 'cactus-template',
			array( 'cactusTemplater', 'render_admin_page' )
		);
		...
		<ul class="cactus-list-md-3">
		  <li>
			<div class="form-group">
			  ...
			  <input type="text" class="form-control" id="name" placeholder="<?php echo apply_filters('cc_label_name',__( 'Name', 'cactus-companion' ));?> *">
			</div>
		  </li>
		  <li>
			<div class="form-group">
			  <label for="email" class="control-label sr-only"><?php _e( 'Email', 'cactus-companion' );?></label>
			  <input type="email" class="form-control" id="email" placeholder="<?php echo apply_filters('cc_label_email',__( 'Email', 'cactus-companion' ));?> *">
			</div>
		  </li>
		  <li>
			<div class="form-group">
			  <label for="subject" class="control-label sr-only"><?php _e( 'Subject', 'cactus-companion' );?></label>
			  <input type="text" class="form-control" id="subject" placeholder="<?php echo apply_filters('cc_label_subject',__( 'Subject', 'cactus-companion' ));?> *">
			</div>
		  </li>
		</ul>
		<div class="form-group">
		  <label class="control-label sr-only" for="message"><?php echo apply_filters('cc_label_message',__( 'Message', 'cactus-companion' ));?></label>
		  <textarea name="message" id="message" required="required" aria-required="true" rows="4" placeholder="<?php echo apply_filters('cc_label_message',__( 'Message', 'cactus-companion' ));?> *" class="form-control"></textarea>
		</div>
		<div class="form-group">
		  <label class="control-label sr-only" for="submit"><?php echo apply_filters('cc_label_submit',__( 'Submit', 'cactus-companion' ));?></label>
		  <input type="submit" value="<?php echo apply_filters('cc_button_text',__( 'SEND YOUR MESSAGE', 'cactus-companion' ));?>" id="submit">&nbsp;&nbsp;&nbsp;&nbsp;<span class="noticefailed"></span>
		</div>
